...
export async function GET(request: NextRequest) {
	try {
		...
		for (const recurringEvent of recurringEvents) {
			...
			// Convert recurring event dates to local dates for comparison
			// recurringEvent.startDate is stored as UTC, convert to local date for comparison
			const eventStartUTC = new Date(recurringEvent.startDate);
			const eventStartLocal = startOfDay(
				new Date(
			...
			const eventEndLocal = recurringEvent.endDate
				? (() => {
						const eventEndUTC = new Date(recurringEvent.endDate);
						return startOfDay(
							new Date(
								eventEndUTC.getUTCFullYear(),
								eventEndUTC.getUTCMonth(),
								eventEndUTC.getUTCDate()
							)
						);
				  })()
				: null;
			...
			while (currentDate <= end) {
				// Skip if before event start (compare local dates)
				if (currentDate < eventStartLocal) {
					currentDate.setDate(currentDate.getDate() + 1);
					continue;
				}

				// Skip if past event end (compare local dates)
				if (eventEndLocal && currentDate > eventEndLocal) {
					break;
				}
				...
				const utcDateStr = `${utcDateFromLocal.getUTCFullYear()}-${String(
					utcDateFromLocal.getUTCMonth() + 1
				).padStart(2, "0")}-${String(utcDateFromLocal.getUTCDate()).padStart(
					2,
					"0"
				)}`;
				...
				if (
					dateMap.has(dateKey) &&
					dateMap.get(dateKey)!.has(recurringEvent._id.toString())
				) {
					currentDate.setDate(currentDate.getDate() + 1);
					continue;
				}
				...
				// Use eventStartLocal for recurrence matching (local date comparison)
				if (
					matchesRecurrence(
						currentDate,
					...
					generatedCards.push({
						...
						order: recurringEvent.order ?? randomOrder(),
						isGenerated: true,
					});
				}
		...
		const uniqueCards = allCards.filter((card, index, self) => {
			if (card.isGenerated) {
				...
				const dateKey =
					parts.length >= 3
						? parts[2]
						: format(parseISO(card.date), "yyyy-MM-dd");

				return !self.some((c) => {
					if (
						c.isGenerated ||
						!c.recurringEventId ||
						c.recurringEventId !== card.recurringEventId
					) {
						return false;
					}
					// Compare dates using UTC components to avoid timezone issues
					const cDateObj = new Date(c.date);
					const cDateStr = `${cDateObj.getUTCFullYear()}-${String(
						cDateObj.getUTCMonth() + 1
					).padStart(2, "0")}-${String(cDateObj.getUTCDate()).padStart(
						2,
						"0"
					)}`;
	...
	try {
		...
		if (updates.completed !== undefined) {
			...
			if (card.recurringEventId) {
				...
				const dateStr = `${eventDate.getUTCFullYear()}-${String(
